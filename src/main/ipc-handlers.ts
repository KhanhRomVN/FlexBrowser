...
export function registerIpcHandlers(): void {
  ...
  ipcMain.handle(
    'chatgpt:ask-via-tab',
    async (_event, tabId: string, prompt: string, accountId: string) => {
      let retryCount = 0
      ...
      if (!webContentsId) {
        console.error('[ask-via-tab] WebContents not found after retries. Registered tabs:', [
          ...tabToWebContentsMap.keys()
        ])
        ...
        if (!webContentsId) {
          console.warn(
            '[ask-via-tab] No account-based ChatGPT tab found, falling back to any ChatGPT tab'
          )
        }
        ...
        if (!webContentsId) {
          ...
          webContentsId =
            findWebContentsIdByUrl('chat.openai.com') || findWebContentsIdByUrl('chat.openai.com')
        }
        ...
        if (!webContentsId && tabToWebContentsMap.size > 0) {
          const [, fallbackWcId] = [...tabToWebContentsMap.entries()][0]
